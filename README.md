...
- **파일 정리** — 등록한 경로의 파일을 자동으로 휴지통 경로로 이동, 보관 기간 도달 시 영구 삭제
- **휴지통 관리** — 보관 중인 항목 조회·복원·즉시 만료 삭제
- **자체 작업 스케줄링** — 위 작업들의 주기 설정·실행 이력 조회
- **태스크 수행 이력** — 모든 자동/수동 작업의 상세 로그 (필터·정렬)

### echo 서비스 연동이 필요한 기능

- **Echo Note 보관함** — 미래의 누군가에게 닿을 메시지를 PC 에 보관, 일정 시간 후 echo 를 통해 발송
...
echo external API 의 호환되는 버전을 echo 운영팀 측 release note 에서 확인하세요. 호환되지 않는 응답을 받을 경우 `ifonly.muse` 는 Echo Note 기능을 비활성하고 사용자에게 업데이트 안내를 표시합니다 (echo 무관 기능은 영향 없음).
...
라이선스는 추후 별도 결정 후 본 repo 에 추가될 예정입니다. 그 전까지는 코드 사용·수정·재배포에 대한 명시적 권한은 부여되지 않습니다 (저작권자 모든 권리 보유).
