...
호환되지 않는 응답을 받을 경우 `ifonly.muse` 는 해당 echo 의존 기능 (Echo Note, alive-check) 을 stub fallback 으로 처리하고 사용자에게 안내를 표시합니다. echo 무관 기능 (파일 정리·휴지통·자체 스케줄러·작업 이력) 은 영향 없이 동작합니다.
